...
export class InventoryService {
  ...
  getDeliveryAgentOptions(): Observable<DeliveryAgentOption[]> {
    return this.http.get<DeliveryAgentOption[]>(`${this.baseUrl}/inventory/agents/options`);
  }

  updateDeliveryStatus(id: number, status: DeliveryStatus, reason?: string, signature?: string, notes?: string): Observable<Delivery> {
    const params = this.buildHttpParams({
      status,
